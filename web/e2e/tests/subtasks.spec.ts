...
test.describe('Recursive Subtasks', () => {
  test('can add and edit subtasks', async ({ authenticatedPage }) => {
    ...
    await expect(detail.panel.getByText('Untitled Subtask')).toBeVisible();

    // Edit its title
    await detail.editSubtaskTitle('Untitled Subtask', 'E2E Subtask 1');
    await expect(detail.panel.getByText('E2E Subtask 1')).toBeVisible();

    // Toggle completion
    await detail.toggleSubtask('E2E Subtask 1');
    
    // Verify progress indicator (1 / X)
    const progress = detail.panel.locator('span', { hasText: '1 /' });
  ...
  test('supports nested subtasks (recursive)', async ({ authenticatedPage }) => {
    ...
    const parentRow = detail.panel.locator('.group').filter({ hasText: 'Parent Sub' });
    await parentRow.getByTitle('Add subtask').click();
    
    // Type child title
    const childInput = detail.panel.locator('input[placeholder="Subtask title..."]');
    await childInput.fill('Child Sub');
    await childInput.press('Enter');

    await expect(detail.panel.getByText('Child Sub')).toBeVisible();
